...
save_binary(df, 'df_saved_video')

#%%
#Reading Dataframe
#For tests you can start here
df = load_binary("df_saved_video")
...
model = train([x[0] for x in df_train['Face Encoding']], df_train['Name'].values, model_save_path)
#%%
#Predict

#Pode ser substituido por cnn// Realizar encoding do frame
unk_face_frame = np.array(df_test['Face Encoding'].iloc[200]).reshape(1, -1)
closest_neighbors = model.kneighbors(unk_face_frame, n_neighbors=5)

y_pred = model.predict(unk_face_frame) #Não é necessário


#%%
candidate_names = get_labels(df_train['Name'].values,  closest_neighbors)
